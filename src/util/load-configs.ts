...
function LoadConfig<T extends ConfigName>(configName: T): GenericConfig<T> {
  if (configName !== 'tokens' && configName !== 'urls') throw new Error(`No such config: ${configName}`);
...
export const LoadUrlsConfig = () => LoadConfig('urls');
